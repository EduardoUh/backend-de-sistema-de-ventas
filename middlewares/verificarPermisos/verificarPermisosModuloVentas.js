...
module.exports.verificarPermisosModuloVentas = (req = request, res = response, next) => {
    ...
    try {
        ...
        if (!moduloVentas) {
            return res.status(401).json({
                ok: false,
                message: 'Sin acceso al módulo de ventas'
            });
        }

        if (method === 'GET' && !moduloVentas.permisos.find(permiso => permiso === 'VER')) {
            return res.status(401).json({
                ok: false,
